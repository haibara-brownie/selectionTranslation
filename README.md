...
不需要额外授权。装好后从开始菜单启动一次 —— 程序会常驻托盘，之后全局快捷键随时可用，
关掉窗口只是收起，不会退出。建议顺手在设置的「通用」页打开**开机自启**，重启后就不用再手动启动了。
...
一条语义约定：**显式敲子命令 = 单次调用**，窗口关了进程就退（终端里跑一次不会留下
看不见的后台进程）；**不带子命令启动**（双击图标 / 开始菜单）= 常驻托盘，关窗口只是收起。
...
三档都选「系统默认」就完全不发 `font-family`，用系统的。**拉丁字体永远只管非汉字
区间**：就算它自带汉字（Maple Mono 就是），也不会影响汉字的显示 —— 中文字体选
「系统默认」时汉字就用系统默认。想让 Maple 连汉字一起管，把中文字体也选成 Maple。
...
| **mac / Windows**：快捷键完全没反应 | 先确认托盘里有 seltrans 图标 —— 快捷键靠常驻进程监听，进程没在跑就不会有反应。从开始菜单 / 启动台启动一次，并在设置里打开开机自启 |
...
| 平台 | 位置 |
|---|---|
| Linux | `~/.local/state/seltrans/seltrans.log` |
| macOS | `~/Library/Application Support/seltrans/seltrans.log` |
| Windows | `%LOCALAPPDATA%\seltrans\seltrans.log` |

（「关于」页的「运行日志」分组里写着实际路径，旁边就有「打开」按钮。）
