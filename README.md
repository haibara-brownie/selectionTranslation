...
**原文和译文区是按字符脚本直接分配的**：汉字一定用中文字体，其余按 拉丁 → 中文 → 后备 的顺序回退。

这一点很重要，因为 CSS 的 `font-family` 是**逐字符**回退的 —— 列表里第一个有该字形的字体就赢了。而 JetBrains Maple Mono 这类字体自带汉字，光靠回退顺序的话选它当拉丁字体就会把「中文字体」那档彻底架空。所以译文区不靠猜，直接给汉字范围打 `GtkTextTag` 指定字体。

界面文字（按钮、标签这些）仍然走回退链。选了自带汉字的拉丁字体时，「拉丁字体」那行的副标题会提示你这一点 —— 程序会用 Pango 排一个「汉」字、看实际挑中的家族是不是它自己来判断。
