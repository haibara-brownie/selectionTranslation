...
mod imp {
    ...
    /// 这边不用 SVG：muda / tray-icon 要的是位图，而 `icons/32x32.png` 打包时本来就在。
    ...
    pub fn spawn(app: &AppHandle) -> Result<Inner, String> {
        ...
        let icon = Image::from_bytes(ICON_PNG).map_err(|e| format!("托盘图标解码失败：{e}"))?;
        ...
        TrayIconBuilder::with_id(TRAY_ID)
            ...
            // macOS 的模板图标只认 alpha 通道：这个图标是两块实心圆角矩形加白色字形
            // 挖空，alpha 几乎是满的，切成模板会变成两坨看不出内容的黑块。要真做成
            // 模板得另配一张单色图（比如 icons/tray-mono.png），那是资源文件的事。
